...
public class Config
{
    ...
    // ---------------------------------------------------------------------------
    // Null means "not yet set by @BeforeSuite" — Config constructor fills from property files only when null.
    ...
    public static String resultsDirectory = "test-output"; // always has a safe default
    ...
    public Config()
    {
        ...
        // If they are still null (beforeSuite hasn't run yet or didn't receive the param),
        ...
        if (environment == null) environment = runTimeProperties.getProperty("environment",  "staging");
        if (country == null)     country     = runTimeProperties.getProperty("country",      "sg");
        if (browserName == null) browserName = runTimeProperties.getProperty("browserName",  "chromium");
        if (projectName == null) projectName = runTimeProperties.getProperty("projectName",  "CustomerFrontend");
        if (groupName == null)   groupName   = runTimeProperties.getProperty("groupName",    "regression");
        if (branchName == null)  branchName  = runTimeProperties.getProperty("branchName",   "main");
        if (appLanguage == null)           appLanguage           = runTimeProperties.getProperty("appLanguage",            "en");
        if (isDebugMode == null)           isDebugMode           = Boolean.parseBoolean(runTimeProperties.getProperty("debugMode",            "false"));
        if (isRemoteExecution == null)     isRemoteExecution     = Boolean.parseBoolean(runTimeProperties.getProperty("isRemoteExecution",    "false"));
        if (isBrowserStackExecution == null) isBrowserStackExecution = Boolean.parseBoolean(runTimeProperties.getProperty("isBrowserStackExecution", "false"));
        ...
        // 4. Subdirectory env+country file (e.g., parameters/staging/staging-sg.properties)
    ...
    public String replaceArgumentsWithRunTimeProperties(String input)
    {
        ...
        while (result.contains("{$") && safety++ < 20)
        {
            ...
            if (replacement == null) replacement = result.substring(start, end + 1); // leave unchanged
    ...
    /**
     * Given a list of data values, returns the first non-null value if all non-null elements are equal.
    ...
    /**
     * Comprehensive end-of-test log. Call from TestBase.afterMethod before closing the browser.
     */
    public void endTest(ITestResult result)
    {
        ...
        System.out.println("Finished test - '" + testcaseName + "' from class - '" + testcaseClass + "' in " + durationStr);
    }
    ...
    public void logAction(String message)           { Log.action(this, message); }

    /** Logs only when debug mode is enabled. */
    public void logCommentForDebugging(String message)
    {
        if (isDebugMode) Log.comment(this, message);
    }
